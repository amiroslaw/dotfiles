...
(def actions
  ...
  {:video      "pueue add -g default --label '%s' -- mpv --profile=stream"
   ...
   :dl-video   (str "pueue add --escape -g dl-video --label '%s' -- yt-dlp --embed-metadata -f bestvideo[height<=1440]+bestaudio/best -o '" yt-dir "%%(title)s.%%(ext)s' ") ;; TODO it doesnt' work
